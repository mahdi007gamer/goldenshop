...
import {
  ...
  Pause, RotateCcw, Download, RefreshCw, Headphones,
...
interface Product {
  ...
  canonicalUrl?: string | null;
  schemaType?: string;
  highlightTagFa?: string | null;
  highlightTagEn?: string | null;
}
...
/**
 * Shared product page rendering logic — used by both /[lang]/products/[slug] routes.
 * Detects language from the [lang] URL parameter (fa/en) and renders bilingual content.
 */
export default function SharedProductPage() {
  const params = useParams();
  const router = useRouter();
  // Detect language directly from URL — more reliable than context for route changes
  const lang: "fa" | "en" = (params as { lang?: string }).lang === "en" ? "en" : "fa";
  const isRTL = lang === "fa";
  ...
  const hasPrice = product != null && product.price != null && product.price > 0;
  ...
  const submitReview = async () => {
    ...
    try {
      ...
      if (json.success) {
        ...
        const pres = await fetch(`/api/products/by-slug/${slug}`);
  ...
  // Display content: EN page → nameEn (fallback to name), FA page → nameFa (fallback to name)
    ...
    <div className="relative min-h-screen text-white" style={{ backgroundColor: "#05080E" }} dir={dir}>
      ...
      <main className="relative pt-24 pb-20 px-4">
        <div className="mx-auto max-w-6xl">
          ...
          {/* ── Banner Hero ─────────────────────────────────────────────── */}
          {product.bannerImage && (
            <div className="relative rounded-2xl overflow-hidden border border-white/5 h-48 sm:h-64 lg:h-80 flex flex-col">
              <div className="absolute inset-0">
                <img src={product.bannerImage} alt={displayName} className="w-full h-full object-cover" />
              </div>
              {/* Overlay ملایم: تاریکی عمومی خفیف برای خوانایی بج‌ها، بدون محو کردن تصویر */}
              <div className="absolute inset-0 bg-[#05080E]/40" />
              <div className="absolute inset-0 bg-gradient-to-r from-[#05080E]/50 to-transparent" />
              {/* Fade لبه پایین بنر: dissolve نرم مستقیم از تصویر به پس‌زمینه صفحه (30% پایینی) */}
              <div className="absolute bottom-0 left-0 right-0 h-[60%] bg-gradient-to-b from-transparent to-[#05080E] z-[1]" />
              {/* Fade ملایم از بالا: تصویر در تاریکی ورود کنه */}
              <div className="absolute top-0 left-0 right-0 h-[25%] bg-gradient-to-b from-[#05080E]/60 to-transparent z-[1]" />

              {/* محتوای بالای بنر: بج‌ها + عنوان + قیمت — همه داخل بنر، توی نا�یه fade قرار می‌گیرند */}
              <div className="relative z-10 flex-1 flex flex-col justify-end">
                {/* بج‌های بالای تصویر */}
                <div className="px-4 sm:px-6 pt-4 sm:pt-6 flex items-start gap-2 flex-wrap">
                  <div className="flex items-center gap-2 flex-wrap">
                    <div
                      className="flex items-center gap-1.5 px-3 py-1 rounded-lg border backdrop-blur-md border-success/30"
                      style={{ backgroundColor: `${statusColor}18` }}
                    >
                      <span className="h-2.5 w-2.5 rounded-full" style={{ background: statusColor, boxShadow: `0 0 10px ${statusColor}` }} />
                ...
                {/* عنوان و قیمت داخل بنر — overlap با ناحیه fade */}
                <div className="px-4 sm:px-6 pb-4 sm:pb-6 pt-3">
                  <h1
                    className="text-2xl sm:text-3xl lg:text-4xl font-black text-white font-display leading-tight"
                  ...
                  <div className="flex items-center gap-4 mt-2 flex-wrap">
                    ...
                    {/* Selling-point: قیمت ساده یا badge */}
                    {(() => {
                      const tag = isFa ? (product.highlightTagFa || product.highlightTagEn) : (product.highlightTagEn || product.highlightTagFa);
                      if (tag) {
                        return <span className="inline-block px-3 py-1 rounded-lg text-xs font-bold border border-gold/40 text-gold bg-gold/10">{tag}</span>;
                      }
                      return null;
                    })()}
                  </div>
          ...
          {/* ── Top section: Gallery + Info — ستونی ── */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 mt-6">
            ...
            {/* Right: Product Info — توضیحات + قیمت + دکمه‌ها */}
            <div className="space-y-5">
              ...
              <div className="flex gap-3 pt-2">
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 py-3.5 text-sm font-bold rounded-xl transition-all duration-200 ${
                    hasPrice ? "hover:shadow-lg hover:scale-[1.02]" : "opacity-40 cursor-not-allowed"
                  }`}
                  style={{
                    background: hasPrice ? "linear-gradient(135deg, #C9963A, #F0C060)" : "rgba(255,255,255,0.05)",
                    color: hasPrice ? "#06090F" : "rgba(255,255,255,0.3)",
                    boxShadow: hasPrice ? "0 4px 20px rgba(201,150,58,0.3)" : "none",
                ...
                <button
                  ...
                  style={{
                    fontFamily: isRTL ? "'Vazirmatn', sans-serif" : "'Rajdhani', sans-serif",
                    borderColor: !hasPrice ? "rgba(255,255,255,0.05)" : undefined,
                    background: !hasPrice ? "rgba(255,255,255,0.02)" : undefined,
                    color: !hasPrice ? "rgba(255,255,255,0.2)" : undefined,
                  }}
                >
              ...
              {/* Unavailable notice */}
              {!hasPrice && (
                <p className="text-xs text-center text-red-400/70 mt-2" style={{ fontFamily: isRTL ? "'Vazirmatn'" : "'Inter', sans-serif" }}>
          ...
          {/* ── Key Features — کل‌عرض زیر Gallery + Info ── */}
          {(displayFeaturesDetail.length > 0 || displayFeatures.length > 0) && (
            <div className="mt-10 pt-6 border-t border-white/5">
              <h3 className="text-xs font-bold uppercase tracking-widest text-gold/70 mb-4">{t("product.keyFeatures", lang)}</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {displayFeaturesDetail.length > 0 ? (
                  displayFeaturesDetail.map((f, i) => {
          ...
          {/* ── Bottom section: Full Description — کل عرض ── */}
